  ...
  event => {
    ...
    switch (event.action) {
      case 'homepage-action':
        ...
        event.waitUntil(self.clients.openWindow('https://kdt-ai4-team17.elicecoding.com'));
        break;
      case 'checklist-action': // 오늘 날짜의 체크리스트
        const { encryptedToken } = event.notification.data;
        // public 폴더 안에서는 .env 변수 접근 안 됨
        event.waitUntil(self.clients.openWindow(`https://kdt-ai4-team17.elicecoding.com/m/checklist?token=${encryptedToken}`));
        break;
      // no default
    }
  },
